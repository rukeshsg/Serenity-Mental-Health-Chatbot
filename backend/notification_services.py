...
class EmailService:
    def __init__(self):
        self.smtp_host = os.getenv("SERENITY_SMTP_HOST", "smtp.gmail.com")
        self.smtp_port = int(os.getenv("SERENITY_SMTP_PORT", "587"))
        self.smtp_username = os.getenv("SERENITY_SMTP_USERNAME", "")
        self.smtp_password = os.getenv("SERENITY_SMTP_PASSWORD", "")
        self.smtp_use_tls = os.getenv("SERENITY_SMTP_USE_TLS", "true").strip().lower() != "false"
        self.from_email = os.getenv("SERENITY_EMAIL_FROM") or self.smtp_username

    def _ensure_configured(self):
        if not self.smtp_username or not self.smtp_password or not self.from_email:
            raise DeliveryError(
                "Email delivery is not configured. Set SERENITY_SMTP_USERNAME, "
                "SERENITY_SMTP_PASSWORD, and SERENITY_EMAIL_FROM."
            )

    def send_verification_otp(self, recipient_email: str, otp: str, expiry_minutes: int) -> DeliveryResult:
        self._ensure_configured()

        message = EmailMessage()
        message["Subject"] = "Serenity Verification Code"
        ...
        message.set_content(
            ...
            f"**{otp}**\n\n"
            f"This code is valid for the next **{expiry_minutes} minutes**. "
        ...
        try:
            if self.smtp_use_tls:
                ...
                with smtplib.SMTP_SSL(self.smtp_host, self.smtp_port, timeout=20, context=ssl.create_default_context()) as server:
                    server.login(self.smtp_username, self.smtp_password)
                    server.send_message(message)
        except Exception as exc:
            raise DeliveryError(f"Email delivery failed: {exc}") from exc

        logger.info("OTP email sent via SMTP to %s", _mask_email(recipient_email))
        return DeliveryResult(provider="smtp", recipient_masked=_mask_email(recipient_email))
...
class NotificationService:
    ...
    def send_otp_email(self, recipient_email: str, otp: str, expiry_minutes: int) -> DeliveryResult:
        # Read config at send time so startup/import order cannot freeze stale env values.
        return EmailService().send_verification_otp(recipient_email, otp, expiry_minutes)

    def send_emergency_sms(self, recipient_phone: str, message_text: str) -> DeliveryResult:
        return SmsService().send_emergency_support_message(recipient_phone, message_text)
