...
function App() {
  ...
  const AuthField = ({ id, label, helper, children, ...inputProps }) => (
  ...
  const AuthSubmitButton = ({ children }) => (
    <button type="submit" className="auth-primary-btn" disabled={authLoading}>
      {authLoading && <span className="auth-spinner" aria-hidden="true"></span>}
      <span>{children}</span>
    </button>
  );

  const getAuthCardCopy = () => {
    if (authStep === 'otp') {
      return {
        eyebrow: 'Email verification',
        title: authMode === 'signup' ? 'Verify your email' : 'Enter your OTP',
        body: 'We sent a secure code to keep your account protected.'
      };
    }

    if (authStep === 'emergency-contact') {
      return {
        eyebrow: 'Safety setup',
        title: 'Add a support contact',
        body: 'Optional safety support you can update anytime from settings.'
      };
    }

    if (authStep === 'pin-setup') {
      return {
        eyebrow: 'Private account',
        title: 'Create your secure PIN',
        body: 'A short PIN protects your history, trends, and settings.'
      };
    }
    ...
    if (authMode === 'signup') {
      return {
        ...
        body: 'Use email verification to begin your private wellness space.'
      };
    }
    ...
    if (authMode === 'forgot-pin') {
      return {
        ...
        body: 'We will send a one-time code to help you return securely.'
      };
    }
    ...
    return {
      ...
      title: 'Welcome back',
      body: 'Continue to your calm, private wellness companion.'
    };
  };

  const renderAuthCard = () => {
    const cardCopy = getAuthCardCopy();

    return (
      <div className="auth-modal" aria-labelledby="auth-card-title">
        <div className="auth-card-header">
          <div className="auth-logo-wrap">
            <img src={logo512} alt="Serenity logo" className="auth-logo" />
          </div>
          <div>
            <span className="auth-card-eyebrow">{cardCopy.eyebrow}</span>
            <h2 id="auth-card-title">{cardCopy.title}</h2>
            <p>{cardCopy.body}</p>
          </div>
        </div>
        ...
        {authStep === 'email' && (
          <>
            <form className="auth-form" onSubmit={authMode === 'signin' ? handlePinLogin : handleSendOtp}>
              <AuthField
                id="auth-email"
                label="Email address"
                type="email"
              ...
              {authMode === 'signin' && (
                <AuthPasswordField
                  ...
                  onToggle={() => setShowLoginPin((visible) => !visible)}
                  required
                />
              )}

              {pinError && <p className="error" role="alert">{pinError}</p>}

              <AuthSubmitButton>
                {authLoading
                  ? (authMode === 'signin' ? 'Signing in...' : 'Sending OTP...')
                  : (authMode === 'signin' ? 'Login securely' : 'Send OTP')}
              </AuthSubmitButton>
            </form>
        ...
        {authStep === 'otp' && (
          <>
            <form className="auth-form" onSubmit={handleVerifyOtp}>
              <p className="auth-form-note">Sent to <strong>{userId}</strong></p>
              <AuthField
                id="auth-otp"
                label="One-time code"
                type="text"
              ...
              />
              {pinError && <p className="error" role="alert">{pinError}</p>}
              <AuthSubmitButton>{authLoading ? 'Verifying...' : 'Verify OTP'}</AuthSubmitButton>
            </form>
            <p className="auth-helper-text">
              <button type="button" className="auth-inline-link" onClick={() => switchAuthMode(authMode)}>
                Back
        ...
        {authStep === 'emergency-contact' && (
          <div className="emergency-form auth-form">
            <p className="auth-form-note">You can skip this and finish your account setup.</p>
            <label className="phone-field-label" htmlFor="auth-emergency-phone">Mobile</label>
            <div className="phone-input-group">
              <select
                aria-label="Country code"
                className="country-code-input"
            ...
            <AuthField id="auth-emergency-relationship" label="Relationship">
              <select
                id="auth-emergency-relationship"
                value={emergencyRelationship}
                onChange={(e) => setEmergencyRelationship(e.target.value)}
              >
                <option value="">Select relationship</option>
            ...
            {pinError && <p className="error" role="alert">{pinError}</p>}
            <div className="emergency-buttons">
              <button type="button" className="auth-primary-btn" onClick={handleEmergencyContactContinue}>Continue</button>
              <button type="button" className="auth-secondary-btn" onClick={() => { setPinError(''); setAuthStep('pin-setup'); }}>Skip this step</button>
            </div>
          </div>
        )}

        {authStep === 'pin-setup' && (
          <form className="auth-form" onSubmit={handleSetupPin}>
            <AuthField
              id="auth-username"
              label="Username"
              type="text"
              ...
              required
            />
            <AuthField id="auth-gender" label="Gender">
              <select id="auth-gender" value={gender} onChange={(e) => setGender(e.target.value)}>
                {GENDER_OPTIONS.map((option) => (
                  <option key={`auth-${option.value || 'blank'}`} value={option.value}>{option.label}</option>
                ))}
              </select>
            ...
            <AuthPasswordField
              ...
              onToggle={() => setShowSetupPin((visible) => !visible)}
              required
            />
            <AuthPasswordField
              id="auth-confirm-pin"
              ...
              onToggle={() => setShowConfirmPin((visible) => !visible)}
              required
            />
            {pinError && <p className="error" role="alert">{pinError}</p>}
            <AuthSubmitButton>{authLoading ? 'Saving...' : 'Save PIN and login'}</AuthSubmitButton>
          </form>
        )}
      </div>
    );
        ...
        <div className="auth-gate-shell">
          <div className="auth-gate-visual">
            ...
            <div className="auth-showcase-card">
              <div className="auth-showcase-ambient" aria-hidden="true"></div>
              ...
              <div className="auth-showcase-copy">
                ...
                <h2>Feel heard, then find your next gentle step.</h2>
              </div>
            ...
            <div className="auth-gate-benefits">
              <div className="auth-benefit-card">
                <span className="auth-benefit-icon" aria-hidden="true">01</span>
                <strong>Private by default</strong>
                <span>Secure OTP login, PIN protection, and thoughtful support features.</span>
              </div>
              <div className="auth-benefit-card">
                <span className="auth-benefit-icon" aria-hidden="true">02</span>
          ...
          <div className="auth-gate-panel">
            <div className="auth-gate-copy">
              ...
              <h2>Your quiet space for mental wellness.</h2>
              <p>Secure access to compassionate AI conversations, personal trends, and gentle support tools.</p>
            </div>
            {renderAuthCard()}
          </div>
        </div>
